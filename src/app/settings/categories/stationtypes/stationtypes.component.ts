...
export class StationTypesComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this._settingsservice.getEntitiesGage(this.configSettings.stationTypeURL).subscribe(res => {
    ...
    private createNewStationType() {
        ...
        this._settingsservice.postEntityGage(newItem, this.configSettings.stationTypeURL)
    ...
    public saveStationType(u: StationType, i: number) {
        if (u.name === undefined || u.code === undefined) {
            ...
            delete u.isEditing;
            this._settingsservice.putEntityGage(u.id, u, this.configSettings.stationTypeURL).subscribe(
                (resp) => {
                    u.isEditing = false;
    ...
    public deleteStationType(deleteID: number) {
        ...
        if (check) {
            ...
            this._settingsservice.deleteEntityGage(deleteID, this.configSettings.stationTypeURL)
                .subscribe(result => {
